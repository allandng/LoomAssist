...
type NotifFn = (n: { title: string; message?: string; type: 'info' | 'success' | 'warning' | 'error' | 'progress' }) => void;
...
function fireReminder(ev: Event, addNotification: NotifFn) {
  addNotification({
    type: 'info',
